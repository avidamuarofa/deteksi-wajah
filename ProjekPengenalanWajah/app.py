...
# Memanggil model wajah bawaan OpenCVimport cv2
import streamlit as st
import numpy as np

st.title("Projek Deteksi Wajah Otomatis")
st.write("Ambil foto di bawah, hasil deteksi kotak hijau akan langsung muncul otomatis!")

# Memanggil model wajah bawaan OpenCV
cascPath = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
faceCascade = cv2.CascadeClassifier(cascPath)

# Menggunakan kamera bawaan web standar (Paling stabil & aman)
camera = st.camera_input("Silakan Potret Wajah Kamu")

if camera is not None:
    # Mengubah format gambar agar bisa dibaca OpenCV
    ...
    # LANGSUNG MENAMPILKAN HASIL DETEKSI OTOMATIS DI BAWAHNYA
    st.image(frame, caption="Hasil Deteksi Kamera", use_container_width=True)
cascPath = cv2.data.haarcascades + 'haarcascade_frontalface_default.xml'
faceCascade = cv2.CascadeClassifier(cascPath)
